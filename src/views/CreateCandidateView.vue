<template>
  <Dialog :header="`Добавление кандидата`" v-model:visible="displayModal"
    ...
    <div class="field">
      <div class="flex justify-content-between mt-1">
        ...
        <Dropdown :disabled="localStorage.payload.roles === 'kadry'" v-model="localCandidate.checkStatus" :options="statuses" optionLabel="status" optionValue="code"
...
export default {
  ...
  mounted() {
    this.localCandidate.checkStatus = 0;
  },
  ...
  methods: {
    async saveAndCloseModal() {
      try {
        await this.candidateService.createCandidate(this.localCandidate);
